	...
	for device in platform.get_devices():
		...
		prg = cl.Program(ctx, """
			...
			__global const unsigned int *ciph, __global unsigned int *plain)
			{
						...
						for (r = 0; r < 528; r++)
						{
						ciph_temp = (ciph_temp<<1);
						ciph_temp =ciph_temp^bit(ciph_temp,31);
						ciph_temp =ciph_temp^bit(ciph_temp,15);
						ciph_temp =ciph_temp^bit(key_temp,(15-r)&63);
						ciph_temp =ciph_temp^bit(KeeLoq_NLF,g5(ciph_temp,0,8,19,25,30));
						}
